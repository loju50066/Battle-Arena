...
// Inisialisasi Xendit menggunakan Environment Variable dari Vercel (Sangat Aman)
...
export default async function handler(req, res) {
  // Hanya menerima method POST dari website game
  if (req.method !== 'POST') {
  ...
  try {
    // 1. Perhitungan potongan komisi 10% untuk lu sebagai owner
    const komisiOwner = amount * 0.10;
    const jumlahUangDitransfer = amount - komisiOwner;

    // 2. Tembak API Xendit untuk transfer otomatis ke rekening/e-wallet pemain
    ...
    console.log("Xendit Payout Berhasil:", payoutResponse);

    // Kirim laporan sukses balik ke frontend game
    ...
    return res.status(500).json({
      success: false,
      message: error.message || "Gagal memproses pencairan dana di sistem Xendit."
    });
  }
}
